"""Build edgematrix-viz.html from analyze_results.json (no server required)."""
import json
...
results = json.loads((ROOT / "analyze_results.json").read_text(encoding="utf-8"))["results"]
...
# ── Build edges from smart_relations ──────────────────────────────────────────
raw_edges = []
note_ids = {r["note_id"] for r in results}
for r in results:
    for sr in r["metadata"].get("smart_relations", []):
        target = sr.get("target_id") or sr.get("link")
        if not target or target not in note_ids:
            continue
        raw_edges.append({
            "source":        r["note_id"],
            "target":        target,
            "relation_type": sr.get("relation_type", "related"),
            "confidence":    sr.get("confidence", 0.5),
            "provenance":    sr.get("provenance", "wikilink"),
            "narrative_act": sr.get("narrative_act", ""),
        })

# ── Community data (from analyze_results) ─────────────────────────────────────
macro_map    = {}   # note_id -> comm_id
macro_labels = {}   # str(comm_id) -> label

for r in results:
    nid  = r["note_id"]
    cid  = r.get("community_id", -1)
    macro_map[nid] = cid
    tiers = r.get("community_tiers", [])
    if tiers:
        lbl = tiers[0].get("label", f"comm {cid}")
        # Strip the " (N notes)" suffix for cleaner labels
        lbl = lbl.split(" (")[0] if " (" in lbl else lbl
        macro_labels[str(cid)] = lbl
...
# Override with per-note narrative_act when available (richer assignment)
note_act_override = {r["note_id"]: r.get("narrative_act") for r in results if r.get("narrative_act")}
...
for n in raw_nodes:
    ...
    act = note_act_override.get(nid) or acts.get(str(cid), "ketsu")
    node_act[nid] = act
    enriched.append({
        "id":         nid,
...
# ── HTML template (identical to build_viz.py) ──────────────────────────────────
template = """\
<!DOCTYPE html>
<html lang="en">
<head>
...
.tab-bar{display:flex;border-bottom:1px solid #2d3148;background:#141720}
.tab-btn{flex:1;padding:8px 0;font-size:11px;font-weight:600;color:#64748b;background:transparent;border:none;cursor:pointer;letter-spacing:.04em;transition:color .15s}
.tab-btn:hover{color:#94a3b8}.tab-btn.active{color:#a5b4fc;box-shadow:inset 0 -2px 0 #6366f1}
.tab-pane{display:none}.tab-pane.active{display:block}
.arc-block{background:#1e2235;border:1px solid #2d3148;border-radius:6px;padding:10px 12px;margin-bottom:8px;transition:border-color .2s,box-shadow .2s}
.arc-block .arc-act{font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;margin-bottom:4px}
.arc-block .arc-text{font-size:11px;color:#64748b;line-height:1.5}
.arc-locked{border-color:#6366f1;box-shadow:0 0 0 1px #6366f133,0 0 10px #6366f122}
.arc-locked .arc-text{color:#cbd5e1}
.warning-banner{display:none;margin:8px 10px;padding:7px 10px;background:#7c2d1222;border:1px solid #ef444466;border-radius:5px;font-size:10px;color:#fca5a5;line-height:1.4}
</style>
</head>
<body>
<header>
  ...
  <div class="stat" style="margin-left:auto;color:#4b5563">choracle-remote-00 \u00b7 offline</div>
</header>
<div class="main">
<div id="graph-wrap">
  <svg id="svg"></svg>
...
<div class="sidebar">
  ...
  <div class="tab-bar">
    ...
    <button class="tab-btn" onclick="switchTab('arc')">Arc Generator</button>
  </div>
  ...
  <div class="panel">
    ...
    <p style="font-size:10px;color:#4b5563;margin-bottom:8px">Kish\u014dtenketsu act from per-note analysis results.</p>
    <div class="lr"><div class="sw" style="background:#34d399"></div><div><div class="lbl">ki \u8d77 \u2014 Introduction</div><div class="desc">Largest clusters (\u22657 notes). Foundational, well-connected hubs.</div></div></div>
    ...
    <div class="lr"><div class="sw" style="background:#f59e0b"></div><div><div class="lbl">ten \u8ee2 \u2014 Twist</div><div class="desc">Small clusters (2\u20133 notes). Structural bridges, unexpected pivots.</div></div></div>
    <div class="lr"><div class="sw" style="background:#c084fc"></div><div><div class="lbl">ketsu \u7d50 \u2014 Resolution</div><div class="desc">Singleton clusters. Peripheral; awaiting connection.</div></div></div>
  </div>
  <div class="panel">
    <h2>Relation Type <span style="color:#4b5563">(relation_type)</span></h2>
    <div class="lr"><div class="dsw" style="background:#34d399"></div><div><div class="lbl">supports</div></div></div>
    <div class="lr"><div class="dsw" style="background:#f87171"></div><div><div class="lbl">contradicts</div></div></div>
    <div class="lr"><div class="dsw" style="background:#60a5fa"></div><div><div class="lbl">potential_to</div></div></div>
    <div class="lr"><div class="dsw" style="background:#22d3ee"></div><div><div class="lbl">kinetic_to</div></div></div>
    <div class="lr"><div class="dsw" style="background:#fbbf24"></div><div><div class="lbl">motivates</div></div></div>
    <div class="lr"><div class="dsw" style="background:#fb923c"></div><div><div class="lbl">hinders</div></div></div>
    <div class="lr"><div class="dsw" style="background:#94a3b8"></div><div><div class="lbl">related</div></div></div>
  </div>
  </div>
  <div id="tab-arc" class="tab-pane" style="display:none;padding:14px">
    <div class="arc-block" id="arc-ki">
      <div class="arc-act" style="color:#34d399">起 Ki \u2014 Introduction</div>
      <div class="arc-text">Click generate to draft...</div>
    </div>
    <div class="arc-block" id="arc-sho">
      <div class="arc-act" style="color:#60a5fa">承 Sh\u014d \u2014 Development</div>
      <div class="arc-text">Click generate to draft...</div>
    </div>
    <div class="arc-block" id="arc-ten">
      <div class="arc-act" style="color:#f59e0b">転 Ten \u2014 Twist</div>
      <div class="arc-text">Click generate to draft...</div>
    </div>
    <div class="arc-block" id="arc-ketsu">
      <div class="arc-act" style="color:#c084fc">結 Ketsu \u2014 Resolution</div>
      <div class="arc-text">Click generate to draft...</div>
    </div>
    <button class="btn" style="width:100%;margin-top:4px" onclick="window.generateArc()">Generate Arc</button>
  </div>
</div>
</div>
<script>
...
const sim=d3.forceSimulation(nodes)
  .force('link',d3.forceLink(links).id(d=>d.id).distance(d=>{
    ...
    return d.source.comm===d.target.comm?70:190;
...
const eLine=eSel.append('line')
  ...
  .attr('marker-end',d=>'url(#arr-'+(REL_COLOR[d.relation_type]?d.relation_type:'related')+')');
...
bSel.append('path').attr('class','bg')
  .attr('fill','none')
  .attr('stroke',d=>{const c=ACT_COLOR[d.narrative_act];return c||BRIDGE_COLOR})
  .attr('stroke-width',4).attr('stroke-opacity',0.18).attr('filter','url(#glow)');
const bPath=bSel.append('path').attr('class','bi')
  .attr('fill','none')
  .attr('stroke',d=>{const c=ACT_COLOR[d.narrative_act];return c||BRIDGE_COLOR})
  .attr('stroke-width',2).attr('stroke-opacity',0.85)
  .attr('stroke-dasharray','8,4').attr('marker-end','url(#arr-bridge)');
...
<div class="tr"><span class="tk">target</span><span class="tv" style="font-size:9px">${tgt}</span></div>
...
function showEdgeInfo(d){
  info.style.display='block';
  document.getElementById('info-type').textContent='Edge';
  const c=REL_COLOR[d.relation_type]||'#94a3b8';
...
<div class="if"><span class="ik">target</span><span class="iv" style="font-size:10px;word-break:break-all">${tgt}</span></div>
...
function switchTab(name){
  ...
  document.querySelectorAll('.tab-btn').forEach((b,i)=>b.classList.toggle('active',i===(name==='analysis'?0:1)));
  const pane=document.getElementById('tab-'+name);
  pane.classList.add('active');pane.style.display='block';
}

// \u2500\u2500 Health check (page load) \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
(function(){
  fetch('http://localhost:8000/health')
    .then(r=>r.json())
    .then(d=>{if(!d.ollama_alive)showWarning();})
    .catch(()=>showWarning());
})();
function showWarning(){
  const el=document.getElementById('warning-banner');
  el.textContent='\u26a0\ufe0f Ollama is offline. Arc generation is disabled.';
  el.style.display='block';
}

// \u2500\u2500 Arc generation \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
const ARC_ACTS=['ki','sho','ten','ketsu'];
const lockedActs=new Set();

ARC_ACTS.forEach(act=>{
  document.getElementById('arc-'+act).addEventListener('click',()=>{
    const el=document.getElementById('arc-'+act);
    if(lockedActs.has(act)){lockedActs.delete(act);el.classList.remove('arc-locked');}
    else{lockedActs.add(act);el.classList.add('arc-locked');}
  });
});

window.generateArc=async function(){
  const btn=document.querySelector('#tab-arc .btn');
  btn.textContent='Drafting\u2026';
  btn.disabled=true;
  try{
    const r=await fetch('http://localhost:8000/graph/generate-arc',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
      body:JSON.stringify({locked_acts:[...lockedActs]})
    });
    const data=await r.json();
    ARC_ACTS.forEach(act=>{
      if(!lockedActs.has(act)){
        const text=data[act]||(data.beats&&data.beats[act]);
        if(text)document.querySelector('#arc-'+act+' .arc-text').textContent=text;
      }
    });
  }catch(e){console.error('Arc generation failed:',e);}
  finally{btn.textContent='Generate Arc';btn.disabled=false;}
};
</script>
</body>
</html>"""
...
print(f"  {len(enriched)} nodes, {len(enriched_edges)} edges, {cross_act_count} bridges, {len(sizes)} communities")
