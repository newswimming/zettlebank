...
# How many /graph/ingest-character-graph calls before BERTopic is refit.
# Set to 1 to refit on every ingest (original behaviour).
BERTOPIC_REFIT_INTERVAL: int = int(os.environ.get("BERTOPIC_REFIT_INTERVAL", "1"))
...
# ---------------------------------------------------------------------------
# Graph-version cache — invalidated by _save_graph() on every topology change
# ---------------------------------------------------------------------------

_graph_version: int = 0
# (version, membership, labels) — None until first computation
_macro_cache: tuple[int, dict[str, int], dict[int, str]] | None = None
# (version, constraint_map) — None until first computation
_constraint_cache: tuple[int, dict[str, float]] | None = None
# Number of /graph/ingest-character-graph calls since the last BERTopic refit.
_ingest_count_since_refit: int = 0
...
class StructuralHole(BaseModel):
    ...
    ten_trigger: Optional[str] = Field(
        default=None,
        description="Why Ten candidacy fired: 'low_constraint' | 'is_pivot' | 'symbiote_role' | null.",
    )


class WeightSignals(BaseModel):
    """Decomposition of _compute_act_weights() contributing signals.

    Returned alongside act_weights so the frontend can display per-signal
    candidature evidence without re-running the scoring logic.
    Mirrors schema.ts WeightSignalsSchema — update both together.
    """
    community_anchor: str = Field(
        description="Primary act from macro community assignment (dominant signal).",
    )
    constraint_ten_pull: float = Field(
        default=0.0,
        description="Raw Ten pull added by low Burt constraint (0.0–2.0).",
    )
    place_time_ki_boost: float = Field(
        default=0.0,
        description="Total Ki boost from place/ and time/ tags (0.2 per tag).",
    )
    character_role: Optional[str] = Field(
        default=None,
        description="character_role node attribute value, or null if not a character node.",
    )
    character_role_act: Optional[str] = Field(
        default=None,
        description="Act that received the character_role boost, or null.",
    )
    neighbor_act_counts: dict[str, int] = Field(
        default_factory=dict,
        description="Number of graph neighbors in each act (act -> count).",
    )
...
class AnalyzeResponse(BaseModel):
    ...
    community_members: list[str] = Field(
        default_factory=list,
        description="Other note IDs in the same macro community (excludes self).",
    )
    weight_signals: Optional[WeightSignals] = Field(
        default=None,
        description="Decomposed act-weight signals for candidature transparency.",
    )
...
_MYFOLIO_MODES = {"script", "outline", "brainstorm"}


class MyfolioIngestRequest(BaseModel):
    """Request body for POST /ingest/from-myfolio.

    myfolio passes raw user text and a mode hint derived from its classify
    step.  All character/topic classification runs inside zettlebank using
    the existing spaCy + BERTopic + Ollama pipeline; myfolio does not need
    to perform any extraction.
    """
    input_id: str = Field(
        ...,
        min_length=1,
        max_length=128,
        pattern=_NOTE_ID_PATTERN,
        description="Slugified identifier for this submission (generated by myfolio).",
    )
    text: str = Field(
        ...,
        min_length=1,
        max_length=_CONTENT_MAX,
        description="Raw user text from myfolio (script, outline, or brainstorm).",
    )
    mode: str = Field(
        default="brainstorm",
        description="Content mode hint from myfolio: script | outline | brainstorm.",
    )


class MyfolioNoteResult(BaseModel):
    """One vault note created by /ingest/from-myfolio."""
    note_id: str
    kind: str = Field(description="source | character | topic | relation")
    tags: list[str]
    narrative_act: str


class MyfolioIngestResponse(BaseModel):
    """Response for POST /ingest/from-myfolio."""
    input_id: str
    notes_created: list[MyfolioNoteResult]
    note_count: int
    nodes: int
    edges: int
...
def _save_graph() -> None:
    ...
    Increments _graph_version so cached Leiden and Burt-constraint results
    are invalidated on the next request after any topology change.
    """
...
def _get_macro() -> tuple[dict[str, int], dict[int, str]]:
    """Version-cached wrapper around _detect_macro().

    Returns the cached (membership, labels) tuple when the graph has not
    changed since the last computation.  Invalidated by _save_graph().
    """
    global _macro_cache
    if _macro_cache is not None and _macro_cache[0] == _graph_version:
        return _macro_cache[1], _macro_cache[2]
    membership, labels = _detect_macro()
    _macro_cache = (_graph_version, membership, labels)
...
def _get_constraint_map() -> dict[str, float]:
    """Version-cached wrapper around _build_constraint_map().

    Returns the cached constraint map when the graph has not changed since
    the last computation.  Invalidated by _save_graph().
    """
    global _constraint_cache
    if _constraint_cache is not None and _constraint_cache[0] == _graph_version:
        return _constraint_cache[1]
    result = _build_constraint_map()
    _constraint_cache = (_graph_version, result)
    return result
...
def _assign_macro_acts(
    ...
    if remaining:
        ...
        # Pre-build successor sets for ki/ten nodes only — avoids O(E×C) scan.
        ki_ten_succs: dict[str, set[str]] = {
            u: set(g.successors(u)) for u in ki_ten_nodes if u in g
        }
        ...
        def _ketsu_score(cid: int) -> int:
            ...
            return sum(
                1 for succs in ki_ten_succs.values() for v in succs
...
def _compute_act_weights(
    ...
    constraint_map: dict[str, float],
    node_tags: dict[str, list[str]],
) -> tuple[dict[str, float], "WeightSignals"]:
    """Soft per-note act weights normalised to 1.0, plus a WeightSignals breakdown.
    ...
    Returns (weights, signals) where weights sum to 1.0 (rounded to 4 dp) and
    signals contains the raw per-signal contributions for frontend display.
    """
    ...
    neighbor_act_counts: dict[str, int] = {a: 0 for a in acts}
    if note_id in graph:
        ...
        for neighbor in neighbors:
            ...
            if n_cid is not None:
                ...
                raw[n_act] += 0.25
                neighbor_act_counts[n_act] += 1

    # 4. Tag-based Ki signal
    tags = node_tags.get(note_id, [])
    place_time = sum(1 for t in tags if t.startswith("place/") or t.startswith("time/"))
    place_time_ki_boost = round(place_time * 0.2, 4)
    raw["ki"] += place_time_ki_boost

    # 5. Character role signals from graph node attributes
    role: str | None = None
    role_act: str | None = None
    if note_id in graph:
        role = graph.nodes[note_id].get("character_role") or None
        if role == "locus":
            raw["ki"] += 0.5; role_act = "ki"
        elif role == "mirror":
            raw["ki"] += 0.3; role_act = "ki"
        elif role == "dominant":
            raw["sho"] += 0.3; role_act = "sho"
        elif role == "symbiote":
            raw["sho"] += 0.25; raw["ten"] += 0.15; role_act = "sho"

    signals = WeightSignals(
        community_anchor=primary_act,
        constraint_ten_pull=round(ten_pull * 2.0, 4),
        place_time_ki_boost=place_time_ki_boost,
        character_role=role,
        character_role_act=role_act,
        neighbor_act_counts={k: v for k, v in neighbor_act_counts.items() if v > 0},
    )

    # Normalise to sum to 1.0
    total = sum(raw.values())
    if total <= 0.0:
        return {a: 0.25 for a in acts}, signals
    return {a: round(raw[a] / total, 4) for a in acts}, signals
...
async def analyze(req: AnalyzeRequest):
    ...
    macro, macro_labels = await asyncio.to_thread(_get_macro)
    ...
    constraint_map = _get_constraint_map()
    ...
    # Determine Ten trigger reason for StructuralHole
    ten_trigger: str | None = None
    if bridge_detected:
        ten_trigger = "low_constraint"
    elif graph.nodes.get(req.note_id, {}).get("is_narrative_pivot"):
        ten_trigger = "is_pivot"
    elif graph.nodes.get(req.note_id, {}).get("character_role") == "symbiote":
        ten_trigger = "symbiote_role"

    structural_hole = StructuralHole(
        constraint_score=round(bridge_score, 6),
        is_ten_candidate=bridge_detected,
        ten_trigger=ten_trigger,
    )
    ...
    # Compute per-note act weights + WeightSignals breakdown
    act_weights, weight_signals = _compute_act_weights(
        req.note_id, macro, community_act_map, constraint_map, node_tags
    )

    # Community members (other notes in the same macro community)
    community_members = [
        nid for nid, cid in macro.items()
        if cid == macro_id and nid != req.note_id
    ] if macro_id is not None else []
...
async def ingest_character_graph(req: CharacterGraphIngestRequest):
    ...
    # Debounced: only refit every BERTOPIC_REFIT_INTERVAL ingests to avoid
    # expensive model re-training on rapid sequential calls.
    global _ingest_count_since_refit
    _ingest_count_since_refit += 1
    if _ingest_count_since_refit >= BERTOPIC_REFIT_INTERVAL:
        _ingest_count_since_refit = 0
        await asyncio.to_thread(_fit_bertopic_on_vault, True)
    else:
        logger.info(
            "ingest_character_graph: skipping BERTopic refit (%d/%d)",
            _ingest_count_since_refit, BERTOPIC_REFIT_INTERVAL,
        )
...
async def generate_arc(req: GenerateArcRequest):
    ...
    macro, _macro_labels = _get_macro()
    constraint_map = _get_constraint_map()
    node_tags = _get_all_node_tags()
    community_act_map = _assign_macro_acts(macro, graph, constraint_map, node_tags)
...
@app.post("/ingest/from-myfolio", response_model=MyfolioIngestResponse)
async def ingest_from_myfolio(req: MyfolioIngestRequest):
    """Classify raw text from myfolio and create 1–4 vault notes.

    All extraction runs inside zettlebank — myfolio only parses and routes:
      Stage C (spaCy NER)       → character/, place/, time/, object/ tags
      Stage B (BERTopic+Ollama) → topic/<Label> tag
      Stage D (Ollama)          → affect/<value> tag

    Note budget (1–4, hard-capped):
      1 — source note: always created from the raw input text
      +1 — character note: if spaCy extracts ≥1 PERSON/ORG entity
      +1 — topic note: if BERTopic assigns a topic AND a character note exists
      +1 — relation note: if spaCy extracts ≥2 distinct character entities
    """
    if req.mode not in _MYFOLIO_MODES:
        raise HTTPException(
            status_code=422,
            detail=f"mode must be one of {sorted(_MYFOLIO_MODES)}",
        )

    # ── Stage B + C + D in parallel ─────────────────────────────────────────
    aspect_tags, topic_tags, affect_tags = await asyncio.gather(
        asyncio.to_thread(_run_stage_c_aspects, req.text),
        _run_stage_b_topic(req.text, req.input_id, None, {}),
        _run_stage_d_llm_classify(req.text),
    )

    # Character slugs extracted by spaCy, frequency-ordered (Stage C ordering)
    char_slugs: list[str] = [
        t[len("character/"):] for t in aspect_tags if t.startswith("character/")
    ]
    top_topic_tag: str | None = next(
        (t for t in topic_tags if t.startswith("topic/")), None
    )
    top_topic_slug: str | None = (
        top_topic_tag[len("topic/"):] if top_topic_tag else None
    )
    ...
    # ── Note 1: source note (always) ────────────────────────────────────────
    source_tags = _assemble_tags(topic_tags, aspect_tags, affect_tags, limit=10)
    _upsert_node(req.input_id, {
        "tags": source_tags,
        "source": "myfolio",
        "myfolio_mode": req.mode,
    })
    _store_node_tags(req.input_id, source_tags)
    _tag_yaml = "\n".join(f"  - {t}" for t in source_tags)
    (VAULT_NOTES_DIR / f"{req.input_id}.md").write_text(
        (
            f"---\ntags:\n{_tag_yaml}\nsource: myfolio\n"
            f"smart_relations: []\ncommunity_id: null\nupdated: {now}\n---\n\n"
            f"{req.text.strip()}\n"
    ...
    # ── Note 2: character note (+1 if ≥1 character entity) ──────────────────
    char_note_id: str | None = None
    if char_slugs:
        c1 = char_slugs[0][:120]
        char_note_id = f"char-{c1}"
        char_tags = [f"character/{c1}"] + affect_tags[:2]
        _upsert_node(char_note_id, {
            "tags": char_tags,
            "is_character_node": True,
            "source": "myfolio",
        })
        _store_node_tags(char_note_id, char_tags)
        _char_tag_yaml = "\n".join(f"  - {t}" for t in char_tags)
        (VAULT_NOTES_DIR / f"{char_note_id}.md").write_text(
            (
                f"---\ntags:\n{_char_tag_yaml}\nsource: myfolio\n"
                f"smart_relations: []\ncommunity_id: null\nupdated: {now}\n---\n\n"
                f"Character extracted from [[{req.input_id}]].\n"
            ).strip(),
            encoding="utf-8",
        )

    # ── Note 3: topic note (+1 if topic found AND character note written) ────
    topic_note_id: str | None = None
    if top_topic_slug and char_note_id:
        topic_note_id = f"topic-{top_topic_slug[:121]}"
        t_tags = [top_topic_tag] + affect_tags[:1]
        _upsert_node(topic_note_id, {"tags": t_tags, "source": "myfolio"})
        _store_node_tags(topic_note_id, t_tags)
        _t_tag_yaml = "\n".join(f"  - {t}" for t in t_tags)
        (VAULT_NOTES_DIR / f"{topic_note_id}.md").write_text(
            (
                f"---\ntags:\n{_t_tag_yaml}\nsource: myfolio\n"
                f"smart_relations: []\ncommunity_id: null\nupdated: {now}\n---\n\n"
                f"Topic note for {top_topic_tag}, referenced in [[{req.input_id}]].\n"
            ).strip(),
            encoding="utf-8",
        )

    # ── Note 4: relation note (+1 if ≥2 character entities) ─────────────────
    rel_note_id: str | None = None
    if len(char_slugs) >= 2:
        c1r, c2r = char_slugs[0][:55], char_slugs[1][:55]
        rel_note_id = f"rel-{c1r}-{c2r}"
        r_tags = [f"character/{c1r}", f"character/{c2r}"] + affect_tags[:1]
        rel_edge = EdgeMatrix(
            target_id=f"char-{c2r}",
            relation_type=RelationType.related,
            narrative_act=NarrativeActEnum.sho,
            confidence=0.7,
            provenance=ProvenanceEnum.llm,
        )
        _upsert_node(rel_note_id, {"tags": r_tags, "source": "myfolio"})
        _store_node_tags(rel_note_id, r_tags)
        _upsert_edges(rel_note_id, [rel_edge])
        _r_tag_yaml = "\n".join(f"  - {t}" for t in r_tags)
        _rel_smart = f"  - {{link: char-{c2r}, type: related, confidence: 0.7}}"
        (VAULT_NOTES_DIR / f"{rel_note_id}.md").write_text(
            (
                f"---\ntags:\n{_r_tag_yaml}\nsource: myfolio\n"
                f"smart_relations:\n{_rel_smart}\ncommunity_id: null\nupdated: {now}\n---\n\n"
                f"Relation between [[char-{c1r}]] and [[char-{c2r}]], "
                f"extracted from [[{req.input_id}]].\n"
            ).strip(),
            encoding="utf-8",
        )

    # ── Leiden: single pass across all new nodes ─────────────────────────────
    _save_graph()
    macro, _ = await asyncio.to_thread(_get_macro)
    constraint_map = _get_constraint_map()
    node_tags_map = _get_all_node_tags()
    community_act_map = _assign_macro_acts(macro, graph, constraint_map, node_tags_map)

    def _act_for(nid: str) -> str:
        cid = macro.get(nid)
        return community_act_map.get(cid, "sho") if cid is not None else "sho"

    # ── Assemble results ─────────────────────────────────────────────────────
    notes_created.append(MyfolioNoteResult(
        note_id=req.input_id, kind="source",
        tags=source_tags, narrative_act=_act_for(req.input_id),
    ))
    if char_note_id:
        notes_created.append(MyfolioNoteResult(
            note_id=char_note_id, kind="character",
            tags=char_tags, narrative_act=_act_for(char_note_id),
        ))
    if topic_note_id:
        notes_created.append(MyfolioNoteResult(
            note_id=topic_note_id, kind="topic",
            tags=t_tags, narrative_act=_act_for(topic_note_id),
        ))
    if rel_note_id:
        notes_created.append(MyfolioNoteResult(
            note_id=rel_note_id, kind="relation",
            tags=r_tags, narrative_act=_act_for(rel_note_id),
        ))

    logger.info(
        "ingest_from_myfolio: input_id=%s mode=%s notes=%d chars=%d",
        req.input_id, req.mode, len(notes_created), len(char_slugs),
    )

    return MyfolioIngestResponse(
        input_id=req.input_id,
        notes_created=notes_created,
        note_count=len(notes_created),
        nodes=graph.number_of_nodes(),
        edges=graph.number_of_edges(),
    )
